...
export function validateUserId(req: Request, res: Response, next: NextFunction) {
    ...
    const user_idSchema = Joi.string().pattern(new RegExp("^[0-9a-fA-F]{24}$")).required();
...
export function validateScriptId(req: Request, res: Response, next: NextFunction) {
    ...
    const script_idSchema = Joi.object({
        ...
        scriptIds: Joi.array().items(Joi.string().pattern(new RegExp("^[0-9a-fA-F]{24}$"))).required(),
    });
...
export function validateScriptData(req: Request, res: Response, next: NextFunction) {
    const { user_id } = req.params;
    ...
    const schema = Joi.array().items(ScriptSchema);
      ...
      if (validationResult.error) {
        return res.status(400).json({ error: `${validationResult.error.details[0].message} sam` }); 
      }
    

      const userIdValidationResult = Joi.string().pattern(new RegExp("^[0-9a-fA-F]{24}$")).validate(user_id);

      if (userIdValidationResult.error) {
          return res.status(400).json({ error: userIdValidationResult.error.details[0].message });
      }
      next();
}
